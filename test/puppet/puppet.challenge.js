...
describe("[Challenge] Puppet", function () {
  ...
  before(async function () {
    ...
    const { logs } = await this.uniswapFactory.createExchange(
      this.token.address,
      { from: deployer }
    );
    ...
    await this.token.transfer(attacker, ATTACKER_INITAL_TOKEN_BALANCE, {
      from: deployer,
    });
  ...
  it("Exploit", async function () {
    ...
    // fund attacker contract with some tokens from attacker
  ...
  after(async function () {
    ...
    expect(await balance.current(attacker)).to.be.bignumber.gte(
      this.intialAttackerEthBalance
    );
  });
});
